...
def createLinLevs(xx, dx):
    """Create linear vector."""
    return np.linspace(xx[0], xx[1], int((xx[1]-xx[0])/dx))
...
def drawPotential( axes, mesh, u, x=[-10.0, 50.0], z=[-50.0, 0.0]
    ...
    # xg = np.linspace(x[0], x[1], int((x[1]-x[0])/dx))
    # yg = np.linspace(z[0], z[1], int((z[1]-z[0])/dx))
    xg = createLinLevs(x, dx)
    yg = createLinLevs(z, dx)
    X,Y = np.meshgrid(xg, yg)

    uI = pg.interpolate(mesh, u, X.flat, pg.Vector(len(Y.flat)), Y.flat, verbose)
                        # pg.ListToRVector( list( X.flat ) ),
                        # pg.RVector( len( Y.flat ), 0.0 ),
                        # pg.ListToRVector( list( Y.flat ) ), verbose )
...
pg.checkAndFixLocaleDecimal_point(True)

dn = pg.DataContainer( '21cpp-node.dat' )
dz = pg.DataContainer( '21cpp-1e-6.dat' )
dZ = pg.DataContainer( '21cpp-1e6.dat' )
...
mesh4 = pg.Mesh( 'mesh/world' )
pot4 = pg.RMatrix( 'pot/num-node.bmat')
mesh5 = pg.Mesh( 'mesh/world-cem' )
pot5  = pg.RMatrix( 'pot/num-1e-4.bmat')

print("loaded:", swatch.duration( True ))
lines = intersectionLines( mesh4.findBoundaryByMarker( -9999 ),
                          pg.Plane(pg.RVector3( 0.0, 1.0, 0.0 ), 0.0 ) )
...
for i in createLinLevs( 200/10, 200, 10 ):
    x,y = pg.misc.streamline( mesh4, pot4[0], pg.Pos( 200, 0.0, -i ), 0.011, maxSteps=5000)
    a4.plot( x,y, color = 'black', linewidth = 0.6, linestyle = 'solid' )
    x,y = pg.misc.streamline( mesh4, pot4[0], pg.Pos( -100, 0.0, -i ), 0.011, maxSteps=5000)
    a4.plot( x,y, color = 'black', linewidth = 0.6, linestyle = 'solid' )

    x,y = pg.misc.streamline( mesh5, pot5[0], pg.Pos( 200, 0.0, -i ), 0.005, maxSteps=5000)
    a5.plot( x,y, color = 'black', linewidth = 0.6, linestyle = 'solid' )
    x,y = pg.misc.streamline( mesh5, pot5[0], pg.Pos( -100, 0.0, -i ), 0.005, maxSteps=5000)
    a5.plot( x,y, color = 'black', linewidth = 0.6, linestyle = 'solid' )

for i in createLinLevs( -100+200/10, 200-200/10, 15 ):
    x,y = pg.viewer.mpl.streamline( mesh4, pot4[0], pg.Pos( i, 0.0, -200 ), 0.011, maxSteps=5000)
    a4.plot( x,y, color = 'black', linewidth = 0.6, linestyle = 'solid' )
    x,y = pg.viewer.mpl.streamline( mesh5, pot5[0], pg.Pos( i, 0.0, -200 ), 0.011, maxSteps=5000)
    a5.plot( x,y, color = 'black', linewidth = 0.6, linestyle = 'solid' )


for i in createLinLevs( 65, 120, 8 ):
    x,y = pg.viewer.mpl.streamline( mesh5, pot5[0], pg.Pos( 0.2, 0.0, -i ), 0.005, maxSteps=5000)
    a5.plot( x,y, color = 'black', linewidth = 0.6, linestyle = 'solid' )

for i in createLinLevs( -5, 38, 8 ):
    x,y = pg.viewer.mpl.streamline( mesh4, pot4[0], pg.Pos( i, 0.0, -60 ), 0.005, maxSteps=5000)
    a4.plot( x,y, color = 'black', linewidth = 0.6, linestyle = 'solid' )
...
fig.savefig( "elecs-borehole.pdf" )
